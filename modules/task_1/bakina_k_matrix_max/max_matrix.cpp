...
void get_random_matrix(std::vector<std::vector<int>>* matrix, const int row_count, const int column_count) {
    std::srand(std::time(0));
    for (int i = 0; i < row_count; i++) {
        for (int j = 0; j < column_count; j++) {
            matrix->at(i)[j] = std::rand();
        }
    }
}
...
int linear_search_max_el(std::vector<int>* sub_matrix, const int size) {
    if (size <= 0 )
        return INT_MIN;
    int max_element = sub_matrix->at(0);
    for (int i = 0; i < size; i++) {
        if (sub_matrix->at(i) > max_element)
            max_element = sub_matrix->at(i);
    }
    return max_element;
}
...
int parallel_search_max_el(std::vector<std::vector<int>>* matrix, int row_count, int column_count) {
    ...
    if (proc_rank == 0) {
        ...
        trans_matrix.resize(matrix_size);
        for (int i = 0; i < column_count; i++) {
            for (int j = 0; j < row_count; j++) {
                trans_matrix[i * row_count + j] = matrix->at(j)[i];
            }
        }
    ...
    recvbuf.resize(size_recvbuf);

    MPI_Scatterv(trans_matrix.data(), sendcounts.data(), displs.data(),
        MPI_INT, recvbuf.data(), size_recvbuf, MPI_INT, 0, MPI_COMM_WORLD);
