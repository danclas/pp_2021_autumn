...
#include "../../../modules/task_3/podovinnikov_a_strongin_global_search/strongin_global_search.h"

#define PI 3.14159265

double Q1(double x) { return ((x + 1) * (x + 1) - 2); }
double Q2(double x) { return (std::sin(x)); }
double Q3(double x) { return (-(x + 1) * (x + 1) - 3); }

TEST(Task_Strongin_global_search, Test_1) {
  int size;
  double ans;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  ans = MPI_Strongin(-3.0, 0.0, Q1);
  // std::cout << ans;
...
TEST(Task_Strongin_global_search, Test_3) {
  ...
  double ans;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  ans = MPI_Strongin(PI / 4, 3 * PI / 4, Q2);
