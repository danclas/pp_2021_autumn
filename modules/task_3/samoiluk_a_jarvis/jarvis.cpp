...
std::vector<Point> JarvisMarch_par(const std::vector<Point>& vec,
    ...
    if (size > vec_size) {
        ...
        p_num = vec_size / size;
        extra_p = vec_size % size;
    }

    std::vector<Point> local_vec(p_num);
    std::vector<Point> local_jm, global_jm;

    MPI_Scatter(vec.data(), p_num, structPoint, local_vec.data(), p_num,
        structPoint, 0, MPI_COMM_WORLD);
    local_jm = JarvisMarch_seq(local_vec);

    if (rank != 0) {
        MPI_Send(local_jm.data(), local_jm.size(), structPoint,
        ...
        for (int i = 1; i < size; i++) {
            ...
            int loc_size = local_jm.size();
            local_jm.resize(loc_size + count);
            MPI_Recv(local_jm.data() + loc_size, count, structPoint,
                i, i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        if (size != 1) {
            if (extra_p != 0) {
                ...
                int loc_size = local_jm.size();
                int extra_size = global_jm.size();
                local_jm.resize(loc_size + extra_size);
                for (int i = loc_size, j = 0; i < local_jm.size(); i++, j++) {
