...
std::vector<double> conjugateGradientMethodParallel(double* A, double* b, int n) {
	...
	if (COMM_NEW != MPI_COMM_NULL) 
	{
		...
		{
			displs[i] = static_cast<int>((static_cast<int>(n * n / size) * i) / n) * n;
		}
		...
		MPI_Scatterv(A, sendcounts.data(), displs.data(), MPI_DOUBLE, aa.data(), sendcounts[rank], MPI_DOUBLE, 0, COMM_NEW);
		std::vector<double> delta(n);
		...
		for (int p = 0; p < n;)
		{
			...
			recvcounts[size - 1] = n;
			MPI_Gatherv(res.data(), end, MPI_DOUBLE, g_k.data(), recvcounts.data(), displs.data(), MPI_DOUBLE, 0, COMM_NEW);
			...
			recvcounts[size - 1] = n;
			MPI_Gatherv(res.data(), end, MPI_DOUBLE, r1.data(), recvcounts.data(), displs.data(), MPI_DOUBLE, 0, COMM_NEW);
			...
			if (p > 0)
			{
				...
				recvcounts[size - 1] = n;
				MPI_Gatherv(res.data(), end, MPI_DOUBLE, r2.data(), recvcounts.data(), displs.data(), MPI_DOUBLE, 0, COMM_NEW);
