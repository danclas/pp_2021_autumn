...
int calcValueByRank(double& value, int rank)
{
    int power = pow(10, rank);
    return (((int)(value * 100)) / power) % 10;
}
...
std::vector<double> SequentialRadixSort(std::vector<double>& vec) {
    ...
    for (int i = 0; i <= maxim_rank; ++i) {
        ...
        for (std::vector<double>::iterator it = (&resultVec)->begin(); it != (&resultVec)->end(); ++it) {
...
std::vector<double> simpleMerge(std::vector<double>& first_vec, std::vector<double>& second_vec)
{
...
std::vector<double> ParallelRadixSort(std::vector<double> global_vec, const std::vector<double>::size_type count_size_vector) {
    int size, rank;
    ...
    if (rank != 0) {
        ...
        for (int i = 1; i < size; ++i) {
            MPI_Recv(localVector.data(), delta, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
            // PROBLEM: OUT OF BOUNDS
            globalVector = simpleMerge(globalVector, localVector);
        }

        if (tail) {
            localVector = std::vector<double>(global_vec.end() - tail, global_vec.end());
            localVector = SequentialRadixSort(localVector);
