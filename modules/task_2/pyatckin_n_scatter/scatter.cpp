...
#include <iostream>
#include <cstring>
#include "./../../modules/task_2/pyatckin_n_scatter/scatter.h"
...
int MyScatter(void* sendbuf, int sendcount,
    ...
    if (sendcount != recvcount || sendcount <= 0 ||
        recvcount <= 0 || root < 0 || root >= size)
    ...
    if (rank == root) {
        ...
        for (int i = 0; i < size; i++) {
            if (i != root) {
                ...
                MPI_Send(static_cast<char*>(sendbuf) +
                    startval * sendtype_size, sendcount, sendtype, i, 1, comm);
            }
        }
    } else {
        MPI_Status status;
